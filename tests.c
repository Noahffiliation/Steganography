...
int main() {
  ...
  failures +=
      run_test("./encode non_existent.bmp out.bmp 'test' > /dev/null 2>&1", 1);

  // Test 3: Output file cannot be created (invalid path)
  failures += run_test(
      "./encode sheldon.bmp /invalid/path/out.bmp 'test' > /dev/null 2>&1", 1);

  // Test 4: Success case - simple message
  failures +=
      run_test("./encode sheldon.bmp test_out.bmp 'hello' > /dev/null 2>&1", 0);

  // Test 5: Success case - empty message
  failures += run_test(
      "./encode sheldon.bmp test_out_empty.bmp '' > /dev/null 2>&1", 0);

  // Test 6: Message that is somewhat long
  failures += run_test(
      "./encode sheldon.bmp test_out_long.bmp 'Testing with a longer message "
      "to cover more bits and loop iterations.' > /dev/null 2>&1",
      0);
